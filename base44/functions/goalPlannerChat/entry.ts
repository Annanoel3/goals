...
1. Generate steps for ALL months in the timeline — detected timeline is ${detectedMonths ? `${detectedMonths} months` : 'INFER from conversation'}.
2. CRITICAL: This plan MUST span every single month from Month 1 through Month ${detectedMonths || '?'}. If any months are missing from the markdown, create them using the pattern from the earliest months.
3. Do NOT skip or abbreviate any months. Do NOT stop early. Return ONLY valid JSON, no markdown fences.
